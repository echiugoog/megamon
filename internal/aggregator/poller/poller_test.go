...
func TestPollResources(t *testing.T) {
	...
	provider := NewPoller(fakeClient, fakeGKE, false, false)
	report := records.NewReport()
	err := provider.PollResources(context.Background(), &report)

	require.NoError(t, err)
	require.NotNil(t, report.JobSetsUp)
	require.NotNil(t, report.NodePoolsUp)
}
